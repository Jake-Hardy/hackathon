...
if (isset($_REQUEST['oid'])) {
	$form->addElement('hidden', 'oid', $_REQUEST['oid']);
}
 
$an =& $form->addElement('select', 'agency', 'Agency', $arrAN, 'onClick="updFood();updToys();"');
  $an->setValue($app['id']);
...
$dobdata = explode("-",dashify($pri['dob']));
...
$pssn_2 =& $form->addElement('text', 'ssn_2', 'SSN', array('size' => 4, 'maxlength' => 4, 'onkeydown' =>"tabIt(this,'down',4)", 'onkeyup' => "tabIt(this,'up',4,this.form.lastNameSp)"));
  $form->addRule('ssn_2', '', 'required');
...
$pssn_0 =& $form->addElement('text', 'ssn_0', 'SSN', array('maxlength' => 9));
  $form->addRule('ssn_0', 'Please enter SSN', 'required');
  $form->addRule('ssn_0', 'SSN must be exactly 9 digits', 'rangelength', array(9,9));
if(!isset($_REQUEST['ssn_0']) || $_REQUEST['ssn_0'] != '***-**-****') {
  $form->addRule('ssn_0', 'SSN must only contain numbers', 'numeric');
}
$pssn_2 =& $form->addElement('text', 'ssn_2', 'SSN', array('maxlength' => 9));
...
$dobdata = explode("-",dashify($sp['dob']));
...
$sssn_2 =& $form->addElement('text', 'ssnSp_2', 'SSN', array('size' => 4, 'maxlength' => 4, 'onkeydown' =>"tabIt(this,'down',4)", 'onkeyup' => "tabIt(this,'up',4,this.form.phone)"));
...
$form->addElement('autocomplete', 'city', 'City', $arrC, array('value' => $app['city'], 'size' => 25, 'maxlength' => 64));
...
*/
	...
	$form->addElement('hidden', 'dob'.$x, '');
        ...
        $ossn_2 =& $form->addElement('text', 'ssn'.$x.'_2', 'SSN', array('size' => 4, 'maxlength' => 4, 'onkeydown' =>"tabIt(this,'down',4)", 'onkeyup' => "tabIt(this,'up',4,this.form.ssn".$x."_0_2)"));
...
*/
	...
	if (!$_POST && !(isset($_REQUEST['oid']))) { 
	 ...
	 if(isset($mem[$x]['wishlist']) && !$_POST && !(isset($_REQUEST['oid']))) {
