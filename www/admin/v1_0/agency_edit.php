...
$form->addElement('hidden', 'oldname', $ag->name);

$form->addElement('text', 'name', 'Name:', array('size' => 25, 'maxlength' => 64, 'value' => $ag->name));
...
$fd =& $form->addElement('select', 'food', 'Food Default', $arrFood);
  $fd->setValue($ag->foodDefault);

$td =& $form->addElement('select', 'toys', 'Toys Default', $arrToys);
  $td->setValue($ag->toysDefault);
...
if ($form->validate()) {
	...
	$ret = Agency::updAgency($id,$form->exportValue('name'),$form->exportValue('food'),$form->exportValue('toys'));
