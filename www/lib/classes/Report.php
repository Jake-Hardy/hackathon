...
class Report {
	...
	static function genAgeReport($uid) {
		$db = new Database();
		return $db->select('SELECT t_agency.name,t_application.toys,
			...
			GROUP BY t_application.agency_id,t_application.toys',SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,SEASON,$uid,SEASON);
	}

	static function genDupeSummaryReport($uid) {
		$db = new Database();
		return $db->select('SELECT t_agency.name,t_agency.id,count(t_application.id) AS count
				...
				AND t_agency.id = t_application.agency_id
				AND t_application.season = ? 
				GROUP BY t_agency.name',$uid,SEASON);
	}

	static function genDupeReport($id) {
                $db = new Database();
/*(?-YEAR(t_fam.dob))-("12-31"<RIGHT(t_fam.dob,5))*/
		return $db->select('SELECT t_fam.id AS individual_id,t_fam.application_id AS family_id,t_fam.type,t_fam.firstName,t_fam.lastName,t_fam.dob,
                        (?-YEAR(t_fam.dob)) AS age,t_fam.sex,t_fam.wishlist,t_ag.name AS agency,t_user.username,
				...
				(SELECT i_ag.name FROM t_family AS i_fam,t_application AS i_app,t_agency AS i_ag
                                ...
                                AND t_app.season = ?
                                AND t_app.dupe = "1"
                                ORDER BY t_fam.application_id',SEASON,$id,SEASON);
	}

	static function genClearedSummaryReport($uid) {
		$db = new Database();
		return $db->select('SELECT t_agency.name,t_agency.id,count(t_application.id) AS count,sum(familySize) AS numInd
				FROM t_application,t_user_agency,t_agency
				WHERE dupe = "0"
				AND t_user_agency.user_id = ?
				AND t_application.agency_id = t_user_agency.agency_id
	...
	static function genClearedReport($id) {
		$db = new Database();
		return $db->select('SELECT t_fam.id AS individual_id,t_fam.application_id AS family_id,t_fam.type,t_fam.firstName,t_fam.lastName,t_fam.dob,
			(?-YEAR(t_fam.dob)) AS age,t_fam.sex,t_fam.wishlist,t_ag.name AS agency,t_user.username,
			t_app.tstamp,t_app.tstampUpd,t_app.season,t_app.phone,t_app.street,t_app.city,t_app.state,t_app.zip,t_app.familySize,
        ...
        static function genDupeSummaryReportAdmin() {
                $db = new Database();
                return $db->select('SELECT t_agency.name,t_agency.id,count(t_application.id) AS count
                                ...
                                AND t_agency.id = t_application.agency_id
                                AND t_application.season = ?
                                GROUP BY t_agency.name',SEASON);
        }

        static function genDupeReportAdmin() {
                $db = new Database();
                return $db->select('SELECT t_fam.id AS individual_id,t_fam.application_id AS family_id,t_fam.type,t_fam.firstName,t_fam.lastName,t_fam.dob,
                        (?-YEAR(t_fam.dob)) AS age,t_fam.sex,t_fam.wishlist,t_ag.name AS agency,t_user.username,
                        t_app.tstamp,t_app.tstampUpd,t_app.season,t_app.phone,t_app.street,t_app.city,t_app.state,t_app.zip,t_app.familySize,
				...
				AND t_ag.id = t_app.agency_id
                                ORDER BY t_fam.application_id',SEASON,SEASON);
        }

        static function genClearedSummaryReportAdmin() {
                $db = new Database();
                return $db->select('SELECT t_agency.name,t_agency.id,count(t_application.id) AS count,sum(familySize) AS numInd
                                FROM t_application,t_agency
                                WHERE dupe = "0"
        ...
        static function genClearedReportAdmin() {
                $db = new Database();
                return $db->select('SELECT t_fam.id AS individual_id,t_fam.application_id AS family_id,t_fam.type,t_fam.firstName,t_fam.lastName,t_fam.dob,
                        (?-YEAR(t_fam.dob)) AS age,t_fam.sex,t_fam.wishlist,t_ag.name AS agency,t_user.username,
                        t_app.tstamp,t_app.tstampUpd,t_app.season,t_app.phone,t_app.street,t_app.city,t_app.state,t_app.zip,t_app.familySize,
	...
	static function genFamilySizeReport($uid) {
		$db = new Database();
		return $db->select('SELECT t_agency.name,
				...
				GROUP BY t_agency.id ORDER BY t_agency.name',SEASON,$uid);
	}

	static function genFoodReport($uid) {
		$db = new Database();
		return $db->select('SELECT t_ag.name,
				...
				AND t_app.dupe = "0"
				GROUP BY t_ag.id',SEASON,$uid);
	}

	static function genToysReport($uid,$under12 = FALSE) {
		$db = new Database();
		...
		return $db->select('SELECT t_ag.name,
				...
				SUM((SELECT COUNT(i_fam.id) FROM t_application AS i_app,t_family AS i_fam
					WHERE toys = "1" AND i_app.id = t_app.id AND i_app.id = i_fam.application_id 
						AND i_fam.id = t_fam.id)) AS toys1c,

				COUNT(DISTINCT(SELECT i_app.id FROM t_application AS i_app
					WHERE toys = "2" AND i_app.id = t_app.id)) AS toys2,				

				SUM((SELECT COUNT(i_fam.id) FROM t_application AS i_app,t_family AS i_fam
					WHERE toys = "2" AND i_app.id = t_app.id AND i_app.id = i_fam.application_id 
					AND i_fam.id = t_fam.id)) AS toys2c
				...
				AND t_fam.type = "3"
				AND ?-YEAR(t_fam.dob) < ?
				GROUP BY t_app.agency_id',SEASON,$uid,SEASON,$age);
	}

	static function genExport($uid) {
                $db = new Database();
                return $db->select('SELECT t_fam.id AS individual_id,t_fam.application_id AS family_id,t_fam.type,t_fam.firstName,t_fam.lastName,t_fam.dob,
                        (?-YEAR(t_fam.dob)) AS age,t_fam.sex,t_fam.wishlist,t_app.agency_id,t_user.username,
                        t_app.tstamp,t_app.tstampUpd,t_app.season,t_app.phone,t_app.street,t_app.city,t_app.state,t_app.zip,t_app.familySize,
                        t_app.employer,t_app.income,t_app.expenses,t_app.comments,t_app.ss,t_app.ssi,t_app.va,t_app.tanf,t_app.fstamps,t_app.other,
				...
				AND t_lk.id = t_fam.lookup_id
                                ORDER BY t_fam.application_id',SEASON,SEASON,$uid);
        }
